...
class XbianDialog(DialogSkinXml):
    ...
    def _loadSettingValue(self,setting):                  
        try :           
           setting.updateFromXbian()                      
            ...
            xbmc.log('XBian-config : Not catched exception(Get xbianValue) : %s - Settings will stay disabled'%(traceback.format_exc()),xbmc.LOGERROR)                                  
        else :
    ...
    def doXml(self,template) :
        xbmc.log('XBian-config : Generate dialog xml from template : %s to %s'%(template,self.xmlfile),xbmc.LOGDEBUG)
