        ...
        if self.menuhasfocus and action in selectAction:
            ...
            if not self.loadingCat[selectCat]:
                ...
                xbmc.executebuiltin('Skin.SetString(%sloadingvalue,%s)' %
                                    (category.getTitle(), 'Loading...'))
                self.loadingCat[selectCat] = True
                initthread = threading.Thread(None, self.onInitThread, None, (category,))
                initthread.start()
                xbmc.log('XBian-config : Loading value thread started for  %s' %
                         category.getTitle(), xbmc.LOGDEBUG)

    def onInitThread(self, category):
        xbmc.log('XBian-config : inthread : Loading value for  %s' %
                 category.getTitle(), xbmc.LOGDEBUG)
        for setting in category.getSettings():
            if self.stopRequested:
                xbmc.log(
                    'XBian-config : Load value Thread stop : user cancel loading', xbmc.LOGDEBUG)
                break
            try:
                xbmc.log('XBian-config : inthread : Start Loading setting value for  %s -> %s' %
                         (category.getTitle(), setting.DIALOGHEADER), xbmc.LOGDEBUG)
                setting.updateFromXbian()
                xbmc.log('XBian-config : inthread : Stop Loading setting value for  %s -> %s' %
                         (category.getTitle(), setting.DIALOGHEADER), xbmc.LOGDEBUG)
                ...
                xbmc.log('XBian-config : Not catched exception in %s (Get xbianValue) : %s - Settings will stay disabled' %
                         (category.getTitle(), traceback.format_exc()), xbmc.LOGERROR)
            else:
                setting.getControl().setEnabled(True)
        xbmc.executebuiltin('Skin.SetString(%sloadingvalue,%s)' % (category.getTitle(), ''))
        xbmc.log('XBian-config : End inthread : Loading value for  %s' %
                 category.getTitle(), xbmc.LOGDEBUG)
